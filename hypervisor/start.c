...
void drawing_desktop()
{
	...
	boxfill8(vram, xsize, COL8_C6C6C6, 0, ysize - 28, xsize - 1,
		 ysize - 28);
	boxfill8(vram, xsize, COL8_FFFFFF, 0, ysize - 27, xsize - 1,
		 ysize - 27);
	...
	if (ret != -1) {
		putfont8_string(vram, xsize, 28, 48, COL8_FFFFFF, font.Bitmap,
				"Memman Free 1 Failed");
	}
		     ...
		     const unsigned char *font_bitmap, unsigned char *string)
{
	...
	for (; y1 < 190; y1 += 30) {
		for (; x1 < 310; x1 += 10) {
			...
			if (ch == 0)
				break;
			putfont8(vram, xsize, x1, y1, color,
				 font_bitmap + (ch - 31) * 16);
			string++;
...
void keyboard_handler(unsigned char data)
{
	...
	if (out_buffer[0] == 0) {
		...
		boxfill8(vram, xsize, COL8_FFFFFF, 3, ysize - 24, 59,
			 ysize - 24);
		boxfill8(vram, xsize, COL8_FFFFFF, 2, ysize - 24, 2, ysize - 4);
		boxfill8(vram, xsize, COL8_848484, 3, ysize - 4, 59, ysize - 4);
		...
		boxfill8(vram, xsize, COL8_848484, xsize - 47, ysize - 24,
			 xsize - 4, ysize - 24);
		boxfill8(vram, xsize, COL8_848484, xsize - 47, ysize - 23,
			 xsize - 47, ysize - 4);
		boxfill8(vram, xsize, COL8_FFFFFF, xsize - 47, ysize - 3,
			 xsize - 4, ysize - 3);
		boxfill8(vram, xsize, COL8_FFFFFF, xsize - 3, ysize - 24,
			 xsize - 3, ysize - 3);

		putfont8_string(vram, xsize, 283, 180, COL8_00FF00, font.Bitmap,
				out_buffer);
#endif
		return;
	}
	...
	putfont8_string(vram, xsize, 0, 0, COL8_FFFFFF, font.Bitmap,
			out_buffer);
...
void mouse_handler(unsigned char data)
{
	...
	if (mouse_status.phase == 0) {
		...
		boxfill8(VRAM_ADDR, xsize, COL8_008484, mouse_status.mx,
			 mouse_status.my, mouse_status.mx + 15,
			 mouse_status.my + 15);
